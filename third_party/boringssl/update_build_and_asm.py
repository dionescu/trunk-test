...
# Based on chromium src third_party/boringssl/update_gypi_and_asm.py
# update_build_and_asm.py

"""Enumerates the BoringSSL source in src/ and generates BUILD file"""
...
# This file is created by update_build_and_asm.py. Do not edit manually.
...
cc_library(
  ...
  srcs = select({
    ":linux32": boringssl_lib_sources + boringssl_linux_x86_sources,
    ":linux64": boringssl_lib_sources + boringssl_linux_x86_64_sources,
    "//conditions:default": boringssl_lib_sources + boringssl_linux_x86_64_sources,
  }),
  defines = [
  ...
  copts = [
    '-Ithird_party/boringssl/src/crypto'
  ],
  linkopts = [
    '-pthread'
  ]
...
def main():
  ...
  with open('BUILD', 'w+') as build_file:
    build_file.write(FILE_HEADER)

    PrintVariableSection(
        build_file, 'boringssl_lib_sources', crypto_c_files + ssl_c_files)

    perlasms = ReadPerlAsmOperations()

    for ((osname, arch), asm_files) in sorted(
        WriteAsmFiles(perlasms).iteritems()):
      PrintVariableSection(build_file, 'boringssl_%s_%s_sources' %
                           (osname, arch), asm_files)

    build_file.write(CC_LIB_RULES)
